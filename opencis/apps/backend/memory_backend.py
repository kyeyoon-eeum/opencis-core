...
class AlignedMemoryBackend:
    ...
    def load(self, addr: int, size: int) -> int:
        ...
        return self._load_fn(addr, size)

    def store(self, addr: int, size: int, value: int):
        addr += self._base_addr
        self._store_fn(addr, size, value)
    ...
    def write_bytes(self, addr: int, data: bytes):
        ...
        for offset in range(0, len(padded), 64):
            ...
            self.store(aligned_start + offset, 64, chunk)
...
class StructuredMemoryAdapter:
    ...
    def store_object(self, obj) -> Tuple[int, int]:
        raw = pickle.dumps(obj)
        raw_len = len(raw)
        addr = self._allocate(raw_len)
        self._backend.write_bytes(addr, raw)
        return addr, raw_len

    def load_object(self, addr: int, size: int):
        raw = self._backend.read_bytes(addr, size)
